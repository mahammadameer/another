...
public class Server  implements ActionListener
{
	...
	public Server() 
	{
		...
		ImageIcon l4=new ImageIcon(ClassLoader.getSystemResource("icons/1.png"));
	...
	public void actionPerformed(ActionEvent ae)
	{
		try
		{
			...
			a1.add(vertical, BorderLayout.PAGE_START); // display message to right side
			if(dout!=null)
			{
			dout.writeUTF(out);
			}
			...
			{
				System.out.println("dout not initilized");
			}
			text.setText("");
			f.repaint();
			f.invalidate(); 
...
public static void main(String []args)
{
	...
	new Server();
	try {
		ServerSocket skt =new ServerSocket(1009);//6001 port number
		while(true)
		{
			Socket s=skt.accept();
